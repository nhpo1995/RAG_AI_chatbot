...
def _validate_documents(documents):
    ...
    for doc in documents:
        ...
        if not doc.content or not str(doc.content).strip():
            logger.warning(f"Skipping document with empty content: {doc.meta.get('filename', 'unknown')}")
            continue 
        # Ensure content is a valid string
        try:
            content_str = str(doc.content).strip()
            if len(content_str) > 0:
                # Create a clean document with validated content
                clean_doc = Document(
                    content=content_str,
                    meta=doc.meta
                )
                ...
                logger.warning(f"Skipping document with empty content after cleaning: {doc.meta.get('filename', 'unknown')}")
        except Exception as e:
            logger.error(f"Error processing document {doc.meta.get('filename', 'unknown')}: {e}")
            continue
...
def safe_embed_documents(documents, batch_size: int = 10):
    ...
    logger.info(f"Starting embedding process for {len(documents)} documents with batch_size={batch_size}")
    ...
    try:
        ...
        # Log some details about the embedded documents
        if embedded_docs:
            first_doc = embedded_docs[0]
            logger.debug(f"First embedded document: {first_doc.meta.get('filename', 'unknown')} - content length: {len(first_doc.content) if first_doc.content else 0}")
